...
#---------------------------------------------------H2A PROCESS FLOW----------------------------------------------------------#
        avg_production_rate = (input_dict.loc[0,'H2 Plant Electrical Input (kW)']/(input_dict.loc[0,'total_system_electrical_usage'] / 24))*input_dict.loc[0,'cap_factor']
        ...
        peak_production_rate = avg_production_rate*(1+input_dict.loc[0,'stack_degradation_oversize'])        # kgH2/day
        ...
        stack_input_power = input_dict.loc[0,'stack_electrical_usage']/24*peak_production_rate/1000             #MW
    ...
    #-------------------------------------------------CAPITAL COST--------------------------------------------------------------#
        ...
        CEPCI_inflator = int((CEPCI.loc[CEPCI['Year']==input_dict.loc[0,'current_year_for_capital_cost'],'CEPCI']))/int((CEPCI.loc[CEPCI['Year']==basis_year_for_capital_cost,'CEPCI']))
        consumer_price_inflator = int(CPI.loc[CPI['Year']==input_dict.loc[0,'current_year_for_capital_cost'],'PCI'])/int(CPI.loc[CPI['Year']==basis_year_for_capital_cost,'PCI'])                   #lookup


    #--------------------------CAPITAL INVESTMENT---------------------------------#
    #----Inputs required in basis year (2016$)----#

        baseline_uninstalled_stack_capital_cost = CEPCI_inflator*consumer_price_inflator*input_dict.loc[0,'system_unit_cost']*total_active_area_degraded*100**2       #($2016)
        ...
        baseline_uninstalled_mechanical_BoP_cost = CEPCI_inflator*consumer_price_inflator*input_dict.loc[0,'mechanical_BoP_unit_cost']*peak_production_rate              #($2016)
        ...
        baseline_uninstalled_electrical_BoP_cost = CEPCI_inflator*consumer_price_inflator*input_dict.loc[0,'electrical_BoP_cost']*stack_input_power*1000             #($2016)
    ...
    def H2CapitalCost(self, input_dict, output_dict):
     ...
     #------------Non Depreciable Capital Costs---------------------#
        ...
        output_dict['land_cost'] = input_dict.loc[0,'cost_of_land']*input_dict.loc[0,'land_required']
      ...
      #--------------------------------------Fixed Operating Costs------------------------------------------------#
        ...
        output_dict['labor_cost'] = input_dict.loc[0,'total_plant_staff']*input_dict.loc[0,'burdened_labor_cost']*2080          #($2016)/year

        input_dict['GA_rate'] = 20                                                     #percent labor cos (general and admin)
        output_dict['GA_cost'] = output_dict['labor_cost']*(input_dict.loc[0,'GA_rate']/100)                               #$/year
        output_dict['licensing_permits_fees'] = 0                                       #$/year

        output_dict['propertytax_insurancecost'] =  total_capital_costs*(input_dict.loc[0,'propertytax_insurancerate']/100)   #$/year
        rent = 0                                                         #$/year
    ...
    def run_module(self):
        avg_production_rate, peak_production_rate, total_active_area_degraded, stack_input_power, total_system_cost = self.Calc_Electrolyzer_Cost(self.input_dict)
        self.ScaledPlantCost(self.input_dict, self.output_dict)
        self.H2CapitalCost(self.input_dict, self.output_dict)
        return avg_production_rate, self.input_dict, self.output_dict
